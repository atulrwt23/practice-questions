...
const firstLettersOfNames = function (objects) {
  return objects.map(extract("name")).map(function (word) { return word[0] });
};
...
console.log(totalPrices([{ price: 10, quantity: 2 },
{ price: 5, quantity: 4 }]));
...
const isAdult = function (objects) {
  return objects.map(extract("age")).map(function (age) { return age >= 18 });
};

console.log(isAdult([{ name: "Alice", age: 17 },
{ name: "Bob", age: 22 }]))
